#!/bin/bash
# Deploy Wazuh Agents to All Infrastructure VMs
# Security Master - Mass Deployment Script
...
DEPLOYMENT_LOG="${LOG_DIR}/wazuh-deployment-${TIMESTAMP}.log"

# Create log directory
mkdir -p "$LOG_DIR"

echo "=========================================" | tee -a "$DEPLOYMENT_LOG"
echo "Wazuh Agent Mass Deployment" | tee -a "$DEPLOYMENT_LOG"
echo "=========================================" | tee -a "$DEPLOYMENT_LOG"
...
for vm_config in "${VMS[@]}"; do
    read -r VMID VM_NAME VM_IP <<< "$vm_config"

    echo "Starting deployment to VM ${VMID} (${VM_NAME})..." | tee -a "$DEPLOYMENT_LOG"

    # Run deployment in background
    {
        "${SCRIPT_DIR}/deploy-wazuh-agent.sh" "$VMID" "$VM_NAME" "$VM_IP" \
            > "${LOG_DIR}/deploy-${VMID}-${TIMESTAMP}.log" 2>&1
        echo $? > "${LOG_DIR}/deploy-${VMID}-${TIMESTAMP}.exitcode"
    } &
...
for i in "${!PIDS[@]}"; do
    ...
    # Read exit code
    EXITCODE=$(cat "${LOG_DIR}/deploy-${VMID}-${TIMESTAMP}.exitcode" 2>/dev/null || echo "1")
...
echo "Success Rate: $(awk "BEGIN {printf \"%.1f%%\", (${SUCCESS_COUNT}/${TOTAL_VMS})*100}")" | tee -a "$DEPLOYMENT_LOG"
echo "" | tee -a "$DEPLOYMENT_LOG"
...
if [ "$FAILED_COUNT" -eq 0 ]; then
    ...
    touch "${LOG_DIR}/deployment-${TIMESTAMP}.success"
    exit 0
else
    ...
    for vm_config in "${VMS[@]}"; do
        ...
        EXITCODE=$(cat "${LOG_DIR}/deploy-${VMID}-${TIMESTAMP}.exitcode" 2>/dev/null || echo "1")
        if [ "$EXITCODE" != "0" ]; then
            echo "  - VM ${VMID} (${VM_NAME}): ${LOG_DIR}/deploy-${VMID}-${TIMESTAMP}.log" | tee -a "$DEPLOYMENT_LOG"
        fi
    done

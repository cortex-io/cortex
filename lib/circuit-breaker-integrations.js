 ...
 *
 * Pre-configured circuit breakers for common operations in cortex:
...
const BREAKER_LOG_DIR = path.join(CORTEX_HOME, 'coordination', 'circuit-breakers');
  ...
  constructor() {
    this.breaker = createApiCircuitBreaker(
      async (workerType, taskId, masterAgent, options = {}) => {
        const scriptPath = path.join(CORTEX_HOME, 'scripts', 'spawn-worker.sh');
